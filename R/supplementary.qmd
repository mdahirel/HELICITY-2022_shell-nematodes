---
title: "Urban _Cepaea nemoralis_ snails are less likely to have nematodes trapped within their shells - supplementary material"
...
library(performance) # CRAN v0.10.8
...
- Interestingly, the differences in built-up surfaces between the three Degree of Urbanisation classes map very closely to built-up thresholds designed and used independently in a series of urban ecology studies from Belgian sites [e.g. @merckxIncreasedBodySize2018; @pianoUrbanizationDrivesCrosstaxon2020] (**Fig. S1-2**). Degree of Urbanisation classes can therefore (_for our data_) be interpreted as encoding both variation in density and in built-up surfaces. 
...
#| message: false
#| output: false
mod=glmmTMB(Nematodes_minus_1 ~ SMOD_type + City + col+scale(nbands)+
              scale(Shell_size) + fusion + (1|Population),data=infected, family=poisson)
...
# conflict between parametric and non parametric overdispersion tests; let's assume overdispersion to be safe
...
mod=glmmTMB(Nematodes_minus_1 ~ SMOD_type + City + col+scale(nbands)+
              scale(Shell_size) + fusion + (1|Population),data=infected, family=nbinom2)
plot(simulateResiduals(mod))
testDispersion(simulateResiduals(mod),alternative="greater")
testDispersion(simulateResiduals(mod),type="PearsonChisq",alternative="greater")
