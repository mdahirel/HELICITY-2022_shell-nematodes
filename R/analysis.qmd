...
(Sidenote: in case of any issue with the geopackage file, an alternative version of the `points_raw` dataset containing exactly the same tabular data, along with explicit coordinate fields in lat/lon and x/y formats, is also available. In which case, you can run the following chunk of code to re-generate the `points` table)
...
aggregated_data <- data |>
  as_tibble() |>
  select(-geom) |> #NB: select(-geometry) if using the csv version of points_raw (cf supra)
